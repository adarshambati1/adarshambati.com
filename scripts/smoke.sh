#!/bin/bash
# End-to-end smoke test for the site: rendering, content collections, the hero
# scenes and PWA wiring.
#
...
# The service worker left with the app; a content site gains nothing from it.
chk "no service worker"     "$(curl -s $B/ | grep -c 'serviceWorker')" 0

echo "== pwa =="
chk "manifest start_url"      "$(curl -s $B/manifest.webmanifest | python3 -c 'import json,sys;print(json.load(sys.stdin)["start_url"])')" /
# sw.js left with the app. Assert it's actually gone, not just unused.
chk "no service worker file"  "$(curl -s -o /dev/null -w %{http_code} $B/sw.js)" 404
chk "icon 192 served"         "$(curl -s -o /dev/null -w %{http_code} $B/icons/icon-192.png)" 200
chk "links manifest"          "$(curl -s $B/ | grep -c 'manifest.webmanifest')" 1
